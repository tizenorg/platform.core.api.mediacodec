...
static void display_sub_basic()
{
    ...
    g_print("z. Auto Configuration Tests\n");
    g_print("=========================================================================================\n");
}
...
void print_list()
{
    ...
    {
        if(list[i].use_video)
            g_printf("[%2d] %s VIDEO %s | %-17s | Width=%-5d Height=%-4d | File=%s [%d]\n", i+1, list[i].use_hw ? "HW" : "SW", list[i].use_encoder ? "Encoder" : "Decoder", list[i].codec_str, list[i].width, list[i].height, list[i].path, i+1);
        else
            g_printf("[%2d] %s AUDIO %s | %-17s | SRate=%-5d Chanel=%-4d | File=%s [%d]\n", i+1, list[i].use_hw ? "HW" : "SW", list[i].use_encoder ? "Encoder" : "Decoder", list[i].codec_str, list[i].width, list[i].height, list[i].path, i+1);
    }
    g_printf("------------------->\n");
}
