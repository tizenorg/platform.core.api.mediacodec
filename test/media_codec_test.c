...
static bool _mediacodec_outbuf_available_cb(media_packet_h pkt, void *user_data)
{
	...
	g_print("file pointer value is  : %p \n", fp_out);
...
static void _mediacodec_prepare(App * app)
{
	...
	g_print("file pointer value at first is  : %p \n", fp_out);
#endif
	/* create instance */
...
static void input_filepath(char *filename, App * app)
{
	...
	app->data = (guint8 *) g_mapped_file_get_bytes(app->file);
	...
	case CURRENT_STATUS_SET_CODEC:
		{
			...
			case 0:
				...
				if (tmp > 100 && (tmp != 112) && (tmp != 128) && (tmp != 144) && (tmp != 160) && (tmp != 161) && (tmp != 162) && (tmp != 163))	//Temporary
				{
...
static void decoder_output_dump(App * app, media_packet_h pkt)
{
	...
	g_print("inside dump, file pointer is %p \n", fp);
