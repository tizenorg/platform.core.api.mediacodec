...
void print_list()
{
    ...
    {
        if(list[i].use_video)
            sprintf(str,"[%%2d] %%s VIDEO %%s | %%-6s | Width=%%-5d Height=%%-4d | File=%%s [%%d]\n");
        else
            sprintf(str,"[%%2d] %%s AUDIO %%s | %%-6s | SRate=%%-5d Chanel=%%-4d | File=%%s [%%d]\n");
        g_printf(str, i+1, list[i].use_hw ? "HW" : "SW", list[i].use_encoder ? "Encoder" : "Decoder",
