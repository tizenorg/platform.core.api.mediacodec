...
mc_ret_e _mc_gst_create_pipeline(mc_gst_core_t *core, gchar *factory_name)
{
	...
	if (core->prepare_count == 0) {
		...
		{
			LOGD("Elements Linking Failed, Parser Allowed = %d, Parser Applied = %d\n", core->is_parser, core->parser?1:0);
			return -1;
		}
		LOGD("Elements Linking Success, Parser Allowed = %d, Parser Applied = %d\n", core->is_parser, core->parser?1:0);
