...
mc_ret_e mc_gst_process_input(mc_handle_t *mc_handle, media_packet_h inbuf, uint64_t timeOutUs)
{
	...
	while (g_atomic_int_get(&core->etb_count) > ENOUGH_DATA) {
		/* still be processing, queuing packet will be blocked until packets have been dropped below NEED_DATA */
