...
int __mc_fill_vdec_packet_with_outbuf(mc_gst_core_t *core, void *data, int size, media_packet_h *pkt)
{
    ...
    if (width == stride_width) {
        ret = media_packet_create_alloc(core->output_fmt, __mc_output_buffer_finalize_cb, core, pkt);
        if (ret != MEDIA_PACKET_ERROR_NONE) {
        ...
        ret = media_packet_get_number_of_video_planes(*pkt, &plane_num);
        if (ret != MEDIA_PACKET_ERROR_NONE) {
            LOGE("media_packet_get_number_of_video_planes failed %d", ret);
            return MC_ERROR;
        }

        buf_size = 0;
        for (i = 0; i < plane_num; i++) {
            media_packet_get_video_plane_data_ptr(*pkt, i, &pkt_data);
            media_packet_get_video_stride_width(*pkt, i, &stride_width);
            media_packet_get_video_stride_height(*pkt, i, &stride_height);

            plane_size = stride_width*stride_height;

            memcpy(pkt_data, data + buf_size, plane_size);
            LOGD("%d plane stride : %d, size : %d", i, stride_width, plane_size);
            buf_size += plane_size;
        }
    } else {
        ...
        for (i = 0; i < tsurf_info.num_planes; i++) {
            tsurf_info.planes[i].stride = i ? stride_width>>1 : stride_width;
            tsurf_info.planes[i].size = i ? (stride_width>>1)*(stride_height>>1) : stride_width*stride_height;

            tsurf_info.planes[i].offset = tsurf_info.size;
        ...
        for (i = 0; i < plane_num; i++) {
            ...
            memcpy(pkt_data, data + buf_size, plane_size);
            LOGD("%d plane size : %d", i, plane_size);
            buf_size += plane_size;
        }
    }
    ret = media_packet_set_buffer_size(*pkt, buf_size);
    if (ret != MEDIA_PACKET_ERROR_NONE) {
        LOGW("media_packet_set_buffer_size failed");
        return MC_ERROR;
    }
...
int __mc_fill_video_packet_with_mm_video_buffer(mc_gst_core_t *core, void *data, int size, media_packet_h *out_pkt)
{
    ...
    if (mm_vbuffer->type == MM_VIDEO_BUFFER_TYPE_PHYSICAL_ADDRESS) {
        ...
        if (tsurf) {
            ret = media_packet_create_from_tbm_surface(core->output_fmt, tsurf,
                    (media_packet_finalize_cb)__mc_output_buffer_finalize_cb, core, out_pkt);
            if (ret != MEDIA_PACKET_ERROR_NONE) {
                LOGW("media_packet_create_from_tbm_surface failed");
                return MC_ERROR;
            }
        }
    }
...
int __mc_fill_packet_with_outbuf(mc_gst_core_t *core, void *data, int size, media_packet_h *out_pkt)
{
    ...
    ret = media_packet_set_buffer_size(*out_pkt, size);
    if (ret != MEDIA_PACKET_ERROR_NONE) {
        LOGW("media_packet_set_buffer_size failed");
        return MC_ERROR;
    }
    ret = media_packet_get_buffer_data_ptr(*out_pkt, &pkt_data);
    if (ret != MEDIA_PACKET_ERROR_NONE) {
        LOGW("media_packet_get_buffer_data_ptr failed");
        return MC_ERROR;
    }
...
GstMCBuffer *_mc_gst_media_packet_to_gstbuffer(mc_gst_core_t* core, GstCaps **caps, media_packet_h pkt, bool codec_config)
{
    ...
    /* pts */
    ret = media_packet_get_pts(pkt, &pts);
    if (ret != MEDIA_PACKET_ERROR_NONE) {
        LOGW("media_packet_get_pts failed");
        return MC_ERROR;
    }
    GST_BUFFER_PTS(mc_buffer->buffer) = pts;

    /* duration */
    ret = media_packet_get_duration(pkt, &dur);
    if (ret != MEDIA_PACKET_ERROR_NONE) {
        LOGW("media_packet_get_duration failed");
        return MC_ERROR;
    }
...
static MMVideoBuffer *__mc_gst_make_tbm_buffer(mc_gst_core_t* core, media_packet_h pkt)
{
    ...
    if (ret != MEDIA_PACKET_ERROR_NONE) {
        LOGW("media_packet_get_tbm_surface failed");
        return MC_ERROR;
    }
