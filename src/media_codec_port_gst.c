...
int new_calc_plane(int width, int height)
{
    ...
    mbX = ALIGN(width, S5P_FIMV_NUM_PIXELS_IN_MB_ROW);
    mbY = ALIGN(height, S5P_FIMV_NUM_PIXELS_IN_MB_COL);
...
static MMVideoBuffer *__mc_gst_make_tbm_buffer(mc_gst_core_t* core, media_packet_h pkt)
{
    ...
    mm_vbuffer->stride_width[1] = mm_vbuffer->stride_width[0];
    mm_vbuffer->stride_height[1] = mm_vbuffer->stride_height[0]>>1;
