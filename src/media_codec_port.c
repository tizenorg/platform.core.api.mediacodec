...
int mc_get_supported_type(MMHandleType mediacodec, mediacodec_codec_type_e codec_type, bool encoder, int *support_type)
{
    ...
    *support_type = 0;
    ...
    const int codec_list = encoder ? (sizeof(encoder_map) / sizeof(encoder_map[0])) :
        (sizeof(decoder_map) / sizeof(decoder_map[0]));
...
int mc_sniff_h264_bitstream(mc_handle_t *handle, media_packet_h pkt)
{
    ...
    if (ret != MEDIA_PACKET_ERROR_NONE) {
        LOGW("media_packet_get_buffer_size failed");
        return MC_ERROR;
    }
    ...
    ret = media_packet_get_codec_data(pkt, &codec_data, &codec_data_size);
    if (ret != MEDIA_PACKET_ERROR_NONE) {
        LOGW("media_packet_get_buffer_data_ptr failed");
        return MC_ERROR;
    }
    ret = media_packet_is_end_of_stream(pkt, &eos);
    if (ret != MEDIA_PACKET_ERROR_NONE) {
        LOGW("media_packet_is_end_of_stream failed");
        return MC_ERROR;
    }
    ret = media_packet_is_codec_config(pkt, &codec_config);
    if (ret != MEDIA_PACKET_ERROR_NONE) {
        LOGW("media_packet_is_codec_config failed");
        return MC_ERROR;
    }
...
int mc_sniff_mpeg4_bitstream(mc_handle_t *handle, media_packet_h pkt)
{
    ...
    ret = media_packet_get_buffer_data_ptr(pkt, (void *)&buf_data);
    if (ret != MEDIA_PACKET_ERROR_NONE) {
        LOGW("media_packet_get_buffer_data_ptr failed");
        return MC_ERROR;
    }
    ...
    ret = media_packet_is_sync_frame(pkt, &sync_frame);
    if (ret != MEDIA_PACKET_ERROR_NONE) {
        LOGW("media_packet_is_sync_frame failed");
        return MC_ERROR;
    }
